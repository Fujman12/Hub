...
def build_blueprint(hub, url_prefix):
    ...
    def endpoint():
        ...
        if mode == 'subscribe':
            ...
            content = get_form_arg('event')
            byte_content = bytes(content, "ascii")
            updated_content = {
                'content':  str(base64.b64encode(byte_content).decode("ascii")),
                'headers': ''
            }
            hub.send_change_notification.delay(topic_url, updated_content)
        else:
